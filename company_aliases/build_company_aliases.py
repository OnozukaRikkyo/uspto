#!/usr/bin/env python3
"""項目2: 会社別名 JSON の生成(会社名の名寄せ)

設計書: /mnt/eightthdd/impact/integrated_data/doc/設計書_項目2_会社名名寄せ.md

段階A: 会社名テーブル構築(IMPACT current_owner + 追加分 disambig_assignee_organization、
...
# ---- 入力(読み取り専用) ----
APPLICANTS_CSV = Path(os.environ.get(
    "CA_APPLICANTS", "/mnt/eightthdd/uspto/patent_applicants.csv"))
...
DATA_DIR = Path(os.environ.get(
    "CA_DATA_DIR", "/mnt/eightthdd/impact/integrated_data/data"))

# ---- 出力 ----
OUT_DIR = Path(os.environ.get(
    "CA_OUT", "/mnt/eightthdd/impact/integrated_data/company_aliases"))
STATE_DIR = OUT_DIR / "state"
DOC_DIR = Path(os.environ.get(
    "CA_DOC", "/mnt/eightthdd/impact/integrated_data/doc"))
REPORT_PATH = DOC_DIR / "名寄せ結果レポート_項目2.txt"
...
def stage_a() -> dict:
    ...
    pairs = []  # (pid7, raw_name)
    pa = pd.read_csv(APPLICANTS_CSV).dropna(subset=["current_owner"])
    for pid, owner in zip(pa["patent_id_normalized"], pa["current_owner"]):
        pairs.append((pad7(pid), owner))
...
def main():
    ...
    for p in (APPLICANTS_CSV, ASSIGNEE_MISSING, DATA_DIR):
        if not p.exists():
