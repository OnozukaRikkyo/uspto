...
def extract_layer1_candidates(prior_art_number, api_key, max_retries=3):
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            for doc in docs:
                if not doc.get("officeActionDate"): continue
...
def verify_layer2_strict(app_number, target_patent, api_key, max_retries=3):
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 上記のフレーズ（法的接着剤）が含まれていない場合は一般文脈のノイズとしてLayer 1に留める
            ...
            return "KEEP_LAYER1", f"API通信エラーのため確認不可 ({e})"
